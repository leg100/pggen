...
func NewArrayType(pgArray pg.ArrayType, caser casing.Caser, elemType Type) Type {
...
//   - *example.com/foo.Qux
//   - []*example.com/foo.Qux
func ParseOpaqueType(qualType string, pgTyp pg.Type) (Type, error) {
	bs := []byte(qualType)
	isArr := bs[0] == '['
	if isArr {
		if bs[1] != ']' {
	...
	if t, ok := typ.(*OpaqueType); !isArr && ok {
		// On array types, the pgTyp goes on the Array. In all other cases, it
	...
	if isArr {
		pgArr, ok := pgTyp.(pg.ArrayType)
		if !ok && pgTyp != nil {
			return nil, fmt.Errorf("opaque pg type %T{%+v} for go type %q is not a pg.ArrayType", pgTyp, pgTyp, qualType)
		}
